...
## 3. Hybrid Strategy: Simulator & TCP
...
**Giải pháp: Hybrid Pattern (Strategy Design Pattern)**
Hỗ trợ 2 chế độ (Mode) song song dựa trên config:
- **TCP Mode**: Dùng cho môi trường Production thực tế. Gọi `net.DialTimeout` để mở socket thực tới IP:Port.
- **Simulator Mode**: Dùng cho môi trường demo/dev. Code không mở socket thực, mà dựa vào trường `uptime_rate` cấu hình cho từng server. 
  - Nếu `uptime_rate = 0.95`, hàm random sẽ trả về Online (Status: ON) 95% thời gian, và 5% trả về Offline. Cố tình nhúng thêm nhiễu (noise) theo hàm lượng giác (Sin/Cos) biến thiên theo giờ để biểu đồ log trên Kibana trông chân thực như biểu đồ máy chủ thực tế trồi sụt vào ban đêm.
